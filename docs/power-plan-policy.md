...
- 撤掉 `IDLEDEMOTE=0` 新写入，保留原方案的降级阈值。
...
- 开关继续开着也会先恢复撤回的 IDLEDEMOTE 和历史 IDLECHECK；仍在使用的两项保持原始快照。
...
i7-9750H 笔记本、RTX 2070 Max-Q、插电、当时的极限档、英雄联盟。同一天两次对照：按上表放开最低性能值到温和值时对局 270 到 280 帧，改回锁 100 后 230 到 250 帧。放开策略维持不变。300 帧以上的差距已定位：候选线程提优默认值由开改关。重新打开后同机同局回到 300 帧以上，重压后台绑核开关对此无影响，默认值已改回开。
