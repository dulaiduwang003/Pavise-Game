...
namespace PaviseApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        {
            new ReleaseNote("1.6.8", "2026-08-07", new[]
            {
                new[]{ "移除「后台 GPU 让位」「网卡中断亲和」「暂停索引 / 预取服务」「保帧线程」四项：前三项无实测数据，第四项台架实测增量落在噪声内。升级后自动还原旧版本写入的值。" },
                new[]{ "移除「禁用全屏优化」：微软已把该路径改进到接近独占全屏，关掉会丢失 Auto HDR 与可变刷新率。" },
                new[]{ "移除「Nagle 与延迟 ACK」：现代游戏的实时流量走 UDP，与 Nagle 无关。" },
                new[]{ "移除「竞技模式禁用 CPU 空闲状态」：部分平台上禁用 C-State 会连带禁用睿频。" },
                new[]{ "移除「MSI 模式」：无法验证设备是否真的支持消息信号中断，写错可能导致设备不工作或无法开机。" },
                new[]{ "「对局前清理待机内存」改为只清低优先级页：清整个待机列表实测丢弃 18894 MB 系统缓存只换来 382 MB 可用内存，耗时也从 1380 毫秒降到 13 毫秒。" },
                new[]{ "设置页新增手动内存清理，执行后列出可用内存与系统缓存两侧的变化。" },
                new[]{ "游戏运行期间后台扫描间隔由 20 秒改为 500 毫秒，新进程更早被压制。" },
                new[]{ "进程快照少一次内核调用，单次耗时由 4.48 毫秒降至 2.65 毫秒。" },
                new[]{ "USB 中断避让不再把中断引向能效核。" },
                new[]{ "显卡中断亲和不再把核显算入，纯核显机器上该开关置灰。" },
                new[]{ "弹窗改为自绘，与主界面同一套视觉。" },
                new[]{ "概览页设备信息条去掉扫描线动画。" },
                new[]{ "「检测到游戏后自动收起窗口」改为默认开启。" },
                new[]{ "更正 README 中「核心分区没有额外贡献」的结论：该结论源自台架未构成竞争，重测后分区在真实竞争下方向为正。" },
            }),
            new ReleaseNote("1.6.7", "2026-08-06", new[]
            {
                new[]{ "电源计划改为自建的「Pavise 竞技」：竞技模式下 CPU 锁 100% 下限、核心不停放、睿频与能效偏好拉满；大小核机器额外接管 E 核与线程调度。本机没有的项自动跳过，退出游戏切回原计划。" },
