    ...
    internal static class ReleaseNotes
    {
        ...
        public static readonly ReleaseNote[] All = new[]
        {
            new ReleaseNote("2.2.2.2", "2026-09-11", new[]
            {
                ...
                new[]{ "新增 启动日志记一行各核的能效档与调度评级", "The startup log now records each core's efficiency and scheduling class.", "起動ログに各コアの効率クラスとスケジューリングクラスを 1 行記録します。" },
                new[]{ "新增 CPU 性能限制的限制标志标出含义 热限 功率限 域依赖", "CPU performance limit flags are now labeled: thermal, power, domain dependency.", "CPU 性能制限のフラグに意味を表示します：熱、電力、ドメイン依存。" },
                new[]{ "新增 概览页公告左边加了捐赠入口 点开是微信二维码 请作者喝杯奶茶 二维码从清单拉 本地有缓存 清单里的 id 没变就不重新下载", "Added a Donate entry to the left of Announcements on the Overview page. It opens a WeChat QR code to buy the author a milk tea. The image comes from the manifest and is cached locally; it is only downloaded again when the manifest id changes.", "概要ページのお知らせの左に「寄付」の入口を追加しました。開くと WeChat の QR コードが表示され、作者にミルクティーを一杯おごれます。画像はマニフェストから取得してローカルに保存し、マニフェストの id が変わったときだけ再取得します。" },
                new[]{ "改进 掌机档不再提供 禁止 CPU 空闲 电源方案空闲策略 缓存预热 显存驻留 核心独占 五项 开着也按关 界面标预设强制关", "The Handheld tier no longer offers five items: disable CPU idle, power plan idle policy, cache warm-up, VRAM residency, and exclusive cores. They resolve to off even when enabled, and the UI marks them forced off by preset.", "ハンドヘルドティアでは CPU アイドル禁止、電源プランのアイドルポリシー、キャッシュ予熱、VRAM 常駐、専有コアの 5 項目を提供しなくなりました。オンでもオフとして扱い、UI にはプリセットによる強制オフと表示します。" },
                new[]{ "改进 其他进程调整了游戏亲和性时 默认保留其改动 本局不再落核 改动后不再覆盖独占范围才撤回独占", "When another process changes the game's affinity, the change is now kept by default and no further placement happens this session; exclusive cores are withdrawn only if the game no longer covers them.", "他のプロセスがゲームのアフィニティを変更した場合、既定ではその変更を維持し今回は以降のコア配置を行いません。ゲームが専有コアを含まなくなった場合のみ専有を撤回します。" },
                new[]{ "改进 中断观测的归因证明容忍线程增删 采集中每轮只核对进程级放置 全量证明三秒一次", "Interrupt observation's attribution proof now tolerates threads starting and exiting; while capturing, each pass checks only process-level placement and the full proof runs every three seconds.", "割り込み観測の帰属証明がスレッドの増減を許容するようになりました。採取中は各回プロセスレベルの配置のみ確認し、完全な証明は 3 秒ごとに行います。" },
                new[]{ "改进 智能档自适应升档只在智能档显示 其它档位整张卡藏起来 逐游戏页同样", "Smart-mode adaptive escalation now only appears in Smart mode; the whole card is hidden in other modes, on the per-game page as well.", "スマートモードの自動昇格はスマートモードでのみ表示します。他のモードではカードごと非表示にし、ゲーム別ページも同様です。" },
                new[]{ "改进 窗口化游戏优化的开关只反映 Pavise 自己的操作 系统本来就开着的显示为关 任何档位都不替用户开", "The windowed game optimization switch now reflects only what Pavise itself did; if the system already has it on, the switch stays off, and no mode turns it on for you.", "ウィンドウ化ゲーム最適化のスイッチは Pavise 自身の操作のみを反映します。システム側で既に有効な場合はオフのまま表示し、どのモードも代わりに有効化しません。" },
                new[]{ "修复 中断观测在线程多的游戏里每几秒重开一次采样窗口 整局出不了实测数据", "Fixed interrupt observation reopening its capture window every few seconds in games with many threads, leaving the session with no measured data.", "スレッド数の多いゲームで割り込み観測が数秒ごとに採取ウィンドウを開き直し、対局中に実測データが得られない問題を修正しました。" },
                new[]{ "修复 中断观测开着时 Pavise 自身 CPU 偏高", "Fixed Pavise's own CPU use running high while interrupt observation is on.", "割り込み観測がオンの間に Pavise 自身の CPU 使用率が高くなる問題を修正しました。" },
                new[]{ "修复 手动落核被其他进程改回后只读不写 读到三次就撤回独占", "Fixed manual placement only reading, never rewriting, after another process changed it back, and withdrawing exclusive cores after three reads.", "手動コア配置が他のプロセスに戻された後、読み取りのみで書き直さず、3 回で専有を撤回していた問題を修正しました。" },
                new[]{ "修复 渲染观测的每一条记录都被日志页归成异常 采样低于门槛或游戏不在前台都是正常记录 只有真故障才进异常", "Fixed every renderer observation line being filed under Errors on the Log page. A sample below the threshold or the game leaving the foreground is a normal record; only real failures count as errors.", "レンダラー観測の全行がログページで異常に分類されていた問題を修正しました。しきい値未満のサンプルやゲームが前面でない場合は通常の記録で、実際の障害だけが異常になります。" },
            }),
            ...
            new ReleaseNote("2.2.1.5", "2026-09-09", new[]
            {
                ...
                new[]{ "改进 核心独占放开 CPU 0 所在物理核 是否独占由用户决定 唯一限制是独占之外至少留两颗完整物理核 默认全选时会占满全部物理核导致开关停用 选核区新增留出系统核快捷键 从末尾整核开始让到刚好成立 说明文字给出总核数已选核数与还需让出的颗数", "Core exclusivity no longer forces CPU 0's physical core out; whether to include it is now your call. The only limit is that two whole physical cores must remain outside the exclusive range. Because the default selection is every core, the switch would always be disabled, so the picker gains a Leave system cores shortcut that releases whole cores from the end until it works, and the text now states the total, the selected count and how many still need releasing.", "コア専有で CPU 0 のある物理コアを一律に除外するのをやめ、含めるかどうかをユーザーが決められるようにしました。制限は「専有範囲の外に完全な物理コアを 2 つ残す」ことだけです。既定では全コアが選択されているためスイッチが必ず無効になるので、選択エリアに「システム用に残す」ショートカットを追加し、末尾の物理コアから成立するまで解放します。説明文には総コア数・選択数・あと何個解放が必要かを表示します。" },
                new[]{ "改进 核心独占按全局一份生效 逐游戏配置里只读显示当前独占摘要 不支持的系统在页面上直接说明要求", "Exclusive cores apply as a single global setting; the per-game config shows the current summary read-only. Systems that cannot use isolation now state the requirement on the page itself.", "コア専有はグローバルに 1 つの設定として適用され、ゲーム個別設定には現在の状況を読み取り専用で表示します。隔離を利用できない環境では、ページ上に必要条件を明示します。" },
                new[]{ "改进 显存驻留退出极限档强制清单 默认关不变 各档位都由用户自己决定开不开", "Video memory residency leaves the Extreme forced list. It stays off by default and every tier now leaves the choice to you.", "ビデオメモリ常駐を極限の強制リストから外しました。既定オフのままで、どのモードでも有効化はユーザーの判断に委ねます。" },
            }),
            new ReleaseNote("2.2.1.4", "2026-09-08", new[]
            {
                new[]{ "改进 游戏高优先级恢复旧规则 限核或核域读不明不再单独降级 CPU 持续饱和且没有生效中的候选提优时才暂回普通 掌机仍不启用候选提优", "Restored the previous game-priority rule: restricted or unknown CPU domains no longer force Normal. Sustained saturation yields only without an active candidate boost; Handheld still excludes candidate boost.", "ゲーム優先度を以前の規則に戻しました。コア制限やコア範囲の不明だけでは通常優先度へ下げず、CPU 飽和が続き候補昇格が有効でない場合のみ下げます。携帯機では候補昇格を引き続き使用しません。" },
                new[]{ "改进 功耗让路取消观察阶段60秒总时限 缺测等待上限从10秒延长到60秒 首次验收最多60秒 证据中断15秒后恢复按无法判定还原", "Power yield observation no longer expires after 60 seconds. Missing evidence can be awaited for up to 60 seconds, with a 60-second initial-verification deadline. Verification resuming after a 15-second evidence gap restores the setting as inconclusive.", "電力配分の観測段階の 60 秒上限を撤廃。欠測待ちと初回検証は最大 60 秒。検証の証拠が 15 秒以上途絶えてから再開した場合は判定不能として元に戻します。" },
                new[]{ "修复 功耗让路仅使用完整配对基线 观察或验收存在缺测与长间隔时负面结果退回但不永久停用", "Power yield uses complete paired baseline samples. Negative results after missing readings or long sampling gaps during observation or verification revert without persistent disablement.", "電力配分の基準値にはそろった標本だけを使用。観測・検証の欠測や長い標本間隔による負の結果は、永続的に無効化せず差し戻します。" },
                new[]{ "修复 功耗让路运行中持续检查有效开关 档位 交流供电和当前托管方案 条件失效会停止并还原", "Power yield rechecks the effective toggle, preset, AC power and active managed plan while running, stopping and restoring when eligibility is lost.", "電力配分が実行中も有効設定・モード・AC 電源・管理対象プランを再確認し、条件を失った場合は停止して復元します。" },
                new[]{ "修复 缓存预热改用稳定会话编号 首次游戏提优完成不再取消预热 换局和退出仍取消旧任务", "Cache warm-up now uses a stable session ID. Completing the first game boost no longer cancels warm-up; a new match or exit still cancels the old task.", "キャッシュ予熱に安定したセッション ID を使用。最初のゲーム昇格完了で予熱が取り消されず、次の対戦や終了時には旧タスクを取り消します。" },
                new[]{ "修复 缓存预热跟随安装目录与渲染路径交接 等待中取消可重新尝试 读取中断不再反复重置预算", "Cache warm-up follows installation and renderer path changes. Cancellation while waiting allows a retry; interrupted reading does not repeatedly reset the budget.", "キャッシュ予熱がインストール先・描画プロセスのパス変更に追従。待機中の取消後は再試行でき、読み取り中断で予算を繰り返しリセットしません。" },
                new[]{ "修复 局中切掌机档或关闭候选线程提优会清收旧车道 还原失败继续重试 残留不再豁免CPU饱和回退", "Switching to Handheld or disabling candidate thread boosting releases the existing lane. Failed restoration is retried, and residual lanes no longer bypass CPU-saturation fallback.", "対戦中に携帯機モードへ切替、または候補スレッド昇格を無効化すると旧レーンを解除。復元失敗は再試行し、残存レーンで CPU 飽和時の降格を回避しません。" },
                new[]{ "改进 Intel核显电源项仅在掌机档且启动时确认唯一显卡为Intel核显时请求最高性能 其他配置保持平衡 CPU大小核自动调度不变", "Intel graphics requests Maximum Performance only in Handheld with a single confirmed Intel iGPU at startup. Other configurations stay Balanced; CPU heterogeneous scheduling stays Automatic.", "起動時に唯一の GPU が Intel 内蔵と確認された携帯機モードのみ、グラフィックスに最大パフォーマンスを要求します。他はバランス、CPU の異種コア調度は自動のままです。" },
                new[]{ "改进 手动核心调度移除重压后台绑核 游戏选核固定显示 保留关超线程 隔离范围单独展开编辑 旧重压设置不再执行", "Simplified manual core scheduling: removed heavy background pinning, kept game selection and No HT visible, and moved isolation selection to an expandable section. Old heavy settings no longer apply.", "手動コア設定を簡素化。高負荷バックグラウンド固定を削除し、ゲーム選択と No HT を常時表示。隔離範囲は展開して編集。旧高負荷設定は適用されません。" },
                new[]{ "改进 点击公告时重新获取内容 按钮立即显示加载中并暂时禁止重复点击 成功后打开公告 失败时提示重试并恢复按钮 无公告时单独提示", "Clicking Announcements now fetches fresh content and immediately shows Loading while blocking repeated clicks. Success opens the announcement; failure restores the button and prompts a retry. An empty feed gets a separate message.", "お知らせをクリックすると最新の内容を取得し、ボタンに読み込み中と表示して連続クリックを防ぎます。成功時はお知らせを開き、失敗時はボタンを戻して再試行を案内。お知らせがない場合は別のメッセージで伝えます。" },
                new[]{ "修复 公告加载中切换语言或重建界面时保留加载状态 收进托盘后不再弹窗或误记已读 晚到的启动结果不再覆盖主动获取的公告", "Announcement loading state survives language changes and interface rebuilds. Completion while hidden in the tray no longer opens a dialog or marks the notice as read, and late startup results cannot overwrite a manual fetch.", "言語切り替えや画面の再構築中も、お知らせの読み込み状態を維持します。トレイ格納後に取得が完了してもダイアログを開かず、既読にもせず、遅れて届いた起動時の結果で手動取得の内容を上書きしません。" },
                new[]{ "文档 删除中英日 README 中过时的旧版性能台架数据及未重测说明", "Removed outdated benchmark figures and the note about missing retests from the Chinese, English and Japanese READMEs.", "中国語・英語・日本語の README から、古いバージョンのベンチマーク数値と再測定未実施の説明を削除しました。" },
            }),
