...
namespace PaviseApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        {
            new ReleaseNote("1.7.0.1", "2026-08-08", new[]
            {
                new[]{ "修复对局中和挂机后按键、鼠标可能变卡：键盘、鼠标、耳机、音频、输入法、手柄类程序（按名字和文件说明识别）不再被压制或冻结。还遇到变卡的，把后台的按键宏类工具加白名单。" },
                new[]{ "手动内存清理改为只清低优先级缓存页，点完不卡电脑。" },
                new[]{ "游戏退出后如有设置没还原成功，不再每 20 秒刷一条日志，改为写明卡在哪一项、静默重试。" },
                new[]{ "新增「极限」模式：在竞技之上，把冻结静默后台、锁核、GPU 让位、电源计划和各项系统开关一次锁到位，模式页里各开关会显示为锁定。键鼠耳机输入法照常不碰。在模式选择里切。" },
                new[]{ "新增「游戏文件预热」，默认关闭：对局开始后把游戏文件悄悄读进缓存，加载和切图更顺。内存不足自动放弃。" },
                new[]{ "新增「对局内存驻留」，默认关闭：内存紧张时把游戏内存钉住不被换出。内存充裕时不动作，退出解除。" },
                new[]{ "新增「后台上传让位」，默认关闭：对局时限制被压制后台的上行带宽，网盘同步、P2P 做种不再拖高延迟。" },
                new[]{ "系统环境页新增「前台时间片校正」：被老优化教程改过的时间片值修回系统默认，正常的不动。" },
                new[]{ "添加游戏支持直接从正在运行的程序里选；切换电源计划不再改变屏幕亮度；启动弹窗重做，新增 QQ 三群 383761286、作者抖音和 B 站入口，弹窗里可以直接检查更新。" },
            }),
            new ReleaseNote("1.7.0", "2026-08-08", new[]
            {
                new[]{ "智能保帧上线，默认开启：单独抬高决定帧数的那个线程，游戏吃满 CPU 时 1% 最差帧改善 77%~96%；识别不出时提优自动降档，负载退去 1 秒内恢复。" },
                new[]{ "「后台 GPU 让位」重新上线：后台吃满显卡时中位帧时间减半、尾部帧改善约四成。" },
                new[]{ "移除 AMD 驱动调优：部分 A 卡有副作用，升级后自动还原相关驱动设置。" },
                new[]{ "新增 QQ 交流二群：1101249532，一群满员加这个。" },
            }),
            new ReleaseNote("1.6.8.1", "2026-08-07", new[]
            {
                new[]{ "「保帧线程」重新上线：补充测试发现 6 核 12 线程这类机型提升明显，游戏吃满 CPU 时 1% 最差帧改善 77%~96%。开关在策略页，沿用你之前的设置。" },
                new[]{ "「竞技模式禁用 CPU 空闲状态」重新上线，新增平台识别：Intel K 系 + Z 板、AMD 桌面 Ryzen + B/X 板可用；其余平台禁 C-State 会连带关闭睿频，开关自动置灰。之前升级把它重置成了关，要用的重新打开。" },
                new[]{ "「暂停索引 / 预取服务」重新上线：机械盘建议开，SSD 不用。" },
                new[]{ "这三项启动时不再被自动清理。" },
            }),
