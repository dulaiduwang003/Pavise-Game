    ...
    internal static class ReleaseNotes
    {
        ...
        public static readonly ReleaseNote[] All = new[]
        {
            new ReleaseNote("2.2.0.0", "2026-09-02", new[]
            {
                ...
                new[]{ "新增 笔记本厂商性能档 Lenovo Legion 与 ASUS ROG 对局中切到性能档 退局切回 电竞与极限档强制", "Added the laptop vendor performance mode for Lenovo Legion and ASUS ROG, switched during a match and back afterwards. Forced in Esports and Extreme.", "ノート PC のベンダー性能モードを追加。Lenovo Legion と ASUS ROG を対局中に性能モードへ切替、退局で戻す。eスポーツと極限で強制。" },
                ...
                new[]{ "修复 极限档电源空闲组的空闲检查周期每局报一项写入失败 已撤掉该项", "Fixed the Extreme idle check period reporting a failed write every match by removing the item.", "極限の電源アイドル確認周期が毎回書き込み失敗を報告する問題を項目撤去で修正。" },
