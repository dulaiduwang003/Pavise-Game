    ...
    internal static class ReleaseNotes
    {
        ...
        public static readonly ReleaseNote[] All = new[]
        {
            new ReleaseNote("2.1.3.1", "2026-08-28", new[]
            {
                ...
                new[]{ "新增 待机内存清理按 ISLC 核心规则工作 列表大小与真正空闲内存同时达到阈值才清理 参数可配置 默认关闭 不裁剪游戏或后台工作集", "Added an optional standby memory cleaner using ISLC's core rules. Both list size and true free memory must meet configured thresholds. Off by default; game and background working sets are not trimmed.", "追加 ISLC の基本ルールに沿うスタンバイメモリ整理。リスト量と実際の空きメモリの両条件を満たした場合だけ整理し、閾値を設定できます。既定はオフで、ゲームやバックグラウンドのワーキングセットは削減しません。" },
