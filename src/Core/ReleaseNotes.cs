...
namespace PaviseApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        public static readonly ReleaseNote[] All = new[]
        {
            ...
            {
                new[]{ "下架极限模式 全套 D3D 台架实测其帧率与竞技完全同档 多出的强制项不产生任何性能收益 竞技即最高档" },
                new[]{ "竞技档插电时禁用处理器闲置 显卡瓶颈下渲染线程不再每帧从深度睡眠唤醒 台架三轮交错实测平均帧 +4.5% 1% Low +6% 连续满载无热衰减 电池上不启用 对局结束还原" },
                new[]{ "修复升级清数据在部分机器上无限中止 驱动预定义的 NVIDIA 逐游戏设置误当作待删残留 精简过电源计划的机器找不到平衡方案切不走托管方案 两者都会卡住清除流程" },
                new[]{ "修复对局电源方案偶发滞留 退出后系统一直用竞技档方案 桌面白吃高功耗 核心停泊还原不再把已切走的方案顶回活动 启动与退出时检测残留自动切回系统方案" },
                new[]{ "常规模式升压提速 持续吃满一个半核以上或狂写盘的后台 约二十秒内直接隔离 台架实测救回损失从 1% 提到 81% 轻负载仍逐级加压不误伤" },
                new[]{ "下架暂停搜索索引与预读服务 固态硬盘上测不出差异 停服务本身要数秒 旧版本停掉的服务升级后自动恢复" },
                new[]{ "下架电源滑块最佳性能 散热受限笔记本上拉满滑块更快撞温度墙 持续性能反降 已切换过的自动还原" },
                new[]{ "修复外设厂商词元混入 host key pad 等通用词 导致 svchost conhost 等大量后台进程被误判为外设而豁免压制" },
                new[]{ "本版本配置结构大改 升级时清除全部旧配置与数据并还原系统改动 一切从默认重新开始" },
                new[]{ "反作弊相容 修复穿越火线一类 TP/内核反作弊游戏对局中掉帧卡死 被保护的游戏不再反复取本体句柄 触发反作弊冻结 后台压制照常" },
                new[]{ "体检页一键修复升级为顶部常驻总按钮 一次修掉检出的全部可修项 无待修项时置灰显示 无需修复" },
                new[]{ "Game DVR 关闭改为游戏模式开启期间常驻 退出才还原 它由系统在游戏启动那刻读取 之前对局中才写对本局无效" },
                new[]{ "电源计划设一次即不再每 30 秒强制拉回 避免与笔记本厂商工具 显卡驱动的电源策略反复抢方案造成周期性顿挫" },
                new[]{ "待机内存清理改为对局中内存吃紧(可用低于 15%)时触发 带 45 秒冷却 默认开启 内存充裕的机器完全无感" },
                new[]{ "AMD 帧率上限改走 Radeon Chill 锁帧 DX9 到 DX12 和 Vulkan 全部生效 老卡不支持时回退帧率目标控制" },
                new[]{ "移除服务让路 它引流的诊断打印类服务本就几乎不吃 CPU 挪核收益接近零" },
                new[]{ "移除后台上传让位 对局中拉 PowerShell 刷 QoS 组策略不稳定 顿挫风险大" },
            }),
            new ReleaseNote("1.8.0.1", "2026-08-14", new[]
            {
                new[]{ "因为呼声过高 游戏专栏回归 目前仅有英雄联盟专栏" },
                new[]{ "新增 AMD 显卡调优" },
                new[]{ "新增逐游戏独立配置 未覆盖跟随全局 生效值在对局激活瞬间定格 对局中的任何修改含自定义核心都是下一局生效" },
                new[]{ "概览页和托盘标注实际生效模式来自哪个游戏" },
                new[]{ "游戏库条目支持重命名 英雄联盟条目带专栏小标" },
                new[]{ "右上角搜索覆盖全部页面含独立配置二级页 命中直达定位 覆盖计数可点击逐项跳转" },
                new[]{ "体检页新增一键修复 MSI 中断 前台时间片 平台时钟 网络限流 键鼠队列检出异常直接修 修完自动复检 可还原" },
                new[]{ "驱动级帧率上限改为滑块 30 到 500 滚轮逐帧微调 NVIDIA 与 AMD 同步" },
                new[]{ "对局电源计划默认 PG 托管方案 首次对局自动创建 可在策略页选回自己的计划" },
                new[]{ "窗口化游戏优化已修复 目前也是回归" },
                new[]{ "游戏档案升到 V5 驱动写入连续失败自动关开关时该游戏的对应覆盖一并清除" },
                new[]{ "修复清除数据时 NVIDIA 驱动项恢复失败仍删快照 现在任何一项还原失败整体中止" },
                new[]{ "修复关闭驱动调优失败后无人重试 启动时自动补还原残留快照" },
                new[]{ "修复关闭 VBS 失败残留快照 之后清数据会改写用户自己设置的 hypervisor 启动项" },
                new[]{ "修复便携标志丢失时数据被搬进本机并删源 现在只复制不删除不覆盖" },
                new[]{ "修复档案首行损坏被当空库 下次保存覆盖掉可恢复内容 现在只读保护并备份" },
                new[]{ "修复扫描 Program Files 这类目录会把整个目录当成一款游戏入库" },
                new[]{ "修复体检 MSI 统计到已拔掉的历史显卡 只统计在场设备" },
                new[]{ "修复 59Hz 与 60Hz 同一物理模式被判为刷新率损失" },
                new[]{ "修复对局中增删白名单和关闭上传限速会卡住界面 改为后台执行" },
                new[]{ "修复联系方式弹窗打开期间空转一个核心" },
                new[]{ "修复 GPU 降频探测在驱动重启后永久失效 体检中断会话在程序被杀后残留" },
                new[]{ "修复若干位图泄漏 亮暗切换会退出独立配置页 托盘菜单不随缩放调整" },
                new[]{ "移除旧版本兼容 V4 及更早档案不再读取 旧版游戏列表不再迁移 程序目录数据不再自动搬家 旧档案一律只读保护不覆写" },
                new[]{ "移除两个开关 禁止前台无输入降级与逐游戏强制独显改为始终开启" },
                new[]{ "移除环境页五个修复型开关 对应能力并入体检页一键修复" },
                new[]{ "移除体检页 NVIDIA 写入实测和鼠标回报率实测" },
            }),
