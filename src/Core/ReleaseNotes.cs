...
namespace PaviseApp
{
    ...
    internal static class ReleaseNotes
    {
        ...
        {
            new ReleaseNote("1.8.0", "2026-08-06", new[]
            {
                new[]{ "新增「不许无故降级」：系统默认会在你一段时间没碰键鼠后给前台程序降一档，手柄游戏和过场动画正好中招，现在可以关掉。" },
                new[]{ "新增「不熄屏不睡眠」：打游戏期间屏幕不会黑，退出自动解除。" },
                new[]{ "新增「电源滑块拉到最高」：打游戏时把系统电源模式切到最佳性能，退出还原原来的档位。" },
                new[]{ "体检页新增节能模式检查：它开着的时候电源模式被锁死，Pavise 的电源优化会失效。" },
                new[]{ "白名单独立成页：把 EXE 或快捷方式拖进来就行，不用再选「应用家族/精确路径/进程名」——作用范围自动判断，命令行和脚本宿主只保护它自己。" },
                new[]{ "白名单能直接从运行中的程序里挑，带图标、内存占用和搜索；列表也换成图标+应用名+实时状态，系统内置项单独分组置底。" },
                new[]{ "构建脚本不再污染终端码页，从命令行跑构建不会再把窗口搞崩。" },
            }),
            new ReleaseNote("1.7.0", "2026-08-06", new[]
            {
                new[]{ "显卡页大扩容（NVIDIA）：DLSS 换新算法、ReBAR 强开、不让 Ansel 进游戏、笔记本电池满血。改过的都记了原值，关掉就还原。" },
                new[]{ "ReBAR 在 BIOS 里开没开会自动检测：开关旁边直接告诉你，体检页也有一行。" },
                new[]{ "新增「后台硬限帧」：打游戏时后台程序统统被限到 20 帧，不许抢显卡；退游戏恢复，就算崩了下次启动也会自动还原。" },
                new[]{ "新增 AMD 驱动调优（实验性）：Anti-Lag、Chill 限帧、Enhanced Sync、RIS 锐化、着色器缓存重置。我们手里没有 A 卡真机，效果等大家反馈。" },
                new[]{ "每局结束告诉你显卡这局被什么拖了后腿：功耗墙、温度墙还是电池；体检页也能实时看。" },
                new[]{ "体检页新增「AMD 写入实测」；NVIDIA 写入实测覆盖全部新项。" },
                new[]{ "修复：帧率上限选 240 会被悄悄变成「关」。" },
            }),
