...
namespace PaviseApp
{
    // Reads this boot's system capability record only on the first scheme configure; no ETW session, no polling
    // The CPPC interface evidence in Event 55 is not the measured HWP MSR state and does not prove the CPPC v2 activity window is usable
    internal static class ProcessorPowerPlatform
    {
        ...
        // null means insufficient evidence; leave the minimum performance value as the scheme has it
        // CPPC plus requested autonomous mode is only used to pick a lower floor, not as proof the hardware is already running autonomously
        internal static bool? AutonomousMinimum(Interface platform, uint? requested)
        {
            ...
            // Single-mode platforms ignore a requested value of 0, so 0 on CPPC does not mean confirmed off either
            return null;
        }
        ...
        internal static Interface Classify(IEnumerable<uint> implementations, int expectedCount)
        {
            ...
            {
                // In Windows Kernel-Processor-Power Event 55, MapPerformanceImplementation
                // 1 is ACPI Performance P States, 3 is ACPI Collaborative Processor Performance Control
        ...
        {
            // Once the UTC clock is adjusted now - uptime drifts from the real boot record, so this is only a loose freshness check
