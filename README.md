...
- 键鼠延迟处理：关闭筛选键和粘滞键（微软的定义是忽略短促击键，开启即引入延迟），禁止系统挂起键鼠设备，修复其它工具改动的输入队列长度
- 4K/8K 鼠标的中断风暴引离游戏核心
- 电源计划、网络、Game DVR、通知和索引服务在游戏结束后全部恢复
- 系统体检页：只读检查本机能力，每条结论标注依据等级。支持一键实测 NVIDIA 写入是否生效、实测鼠标回报率、查看显卡当前受功耗墙或温度墙限制的状态
...
流传的键鼠注册表优化项大多作用在零点几毫秒的量级上，而延迟的主要来源是渲染队列。这类项目 Pavise 只报告不修改，包括蓝牙鼠标、125 Hz 回报率和指针精度增强。

MSI 模式、低延迟模式、后台硬限帧和按位置屏蔽 CPU 0/1 都实现过并已移除，原因分别是实测无效、有写坏设备的风险，以及语义理解错误——驱动中的后台限帧实际作用于失去焦点的应用，切出游戏后被限制的是游戏本身。
...
dev.cmd test   rem 运行内置自测（当前 175 项）
```
...
不允许以任何形式从分发 Pavise 或其修改版中收钱，包括出售拷贝、激活码和下载权限，作为收费商品或订阅服务的组成部分，付费墙、付费解锁和赞赏门槛，预装在收费出售的整机或系统镜像中，以及用它换取流量分成。自愿赞赏不在此列，前提是给不给钱都不影响拿到软件和支持。商业使用需事先邮件取得书面授权。
