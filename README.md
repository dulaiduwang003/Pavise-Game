...
- 三项内存清理默认全部关闭，只建议内存紧张的机器开启，详见下一节
- 对局切到卓越性能电源计划，本机没有会自动创建一份，已有则直接沿用
...
三项内存功能默认全部关闭。**开启后都会带来偶发卡顿**，它们是为内存紧张的低配机准备的。内存充裕的机器开了只有代价没有收益，建议保持关闭。
...
只清系统判定最不可能再用到的那部分缓存页，耗时 13 毫秒，不动整个待机列表，因此不会把仍有价值的文件缓存一起丢掉。每局执行一次。内存紧张的机器加载会顺一点，内存充裕的基本无感知。三项里代价最小的一项。

**对局稳定后回收后台工作集**

开局 30 秒后执行，避开加载阶段，把已被重压的后台程序占用的物理内存交还系统。不使用上表任何一条全局命令，改为对这些进程逐个调用 `SetProcessWorkingSetSize`，游戏、当前前台、白名单和反作弊都不涉及。内存回到待机列表，游戏随时可取，后台程序不会被关闭也不会丢数据。

代价是切回这些程序时第一下会明显停顿，因为它们需要把页面重新读回来。以低优先级分批执行，最长 3 秒，退出对局立即停止。

**待机列表超阈值时清空**

ISLC（Intelligent Standby List Cleaner）的等价实现。对局中每 5 秒读一次待机列表的实际大小，只有待机列表 ≥ 1 GB 并且可用内存 ≤ 1 GB 时才执行清空。

它针对的不是内存不足——上表已经说明清空整个列表几乎不增加可用内存——而是待机列表堆积过大时内存管理器锁争用引起的周期性微卡。代价是文件缓存整个丢弃，之后所有程序读文件都要重新访问硬盘。内存较大的机器基本触发不了这个阈值。
