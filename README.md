...
可逆隔离当前扫描并移动 `Cross`；`DiagnosticAssistant`、`FeedBack`、`NetworkAssist`、`TQM` 和 `TenioDL` 仍是受支持的清单条目，历史批次可以正常恢复与丢弃，但不再纳入新建批次的扫描范围。隔离会将目标移入同盘隐藏仓库，先写清单再移动，不删除、不覆盖；英雄联盟或 WeGame 运行时拒绝操作，遇到目标冲突也不会覆盖已有文件。若客户端更新重新下载了某个组件导致恢复无法覆盖，可在确认每一项都已回到原位置后丢弃该批次记录并重新隔离。
...
内置自测当前包含 `67` 项，覆盖 CPU 拓扑和高低核心数分级、CPU Sets 和硬亲和性恢复、PID 复用、真实子进程提优、分级压制、位号后缀兜底匹配的边界、跨焦点会话保护、前台和用户窗口保护、事件驱动扫描预算、会话边界、英雄联盟凭据解析、精准净化边界、可逆隔离恢复与冲突保护、会话报告、发布元数据和界面离屏绘制。机器缺少对应能力时会记录 `SKIP`，不会算作 `PASS`。
