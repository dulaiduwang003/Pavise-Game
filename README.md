...
自动调度不注入目标进程，不修改其内存，不改其文件；优先级、IO、页面优先级、CPU Sets 和硬亲和性会在接口支持时回读；Power Throttling 没有可靠的进程读取接口，这里只能按写入结果判断。英雄联盟专栏的文件操作只有一种：需要单独确认的可逆隔离，按清单移动附加层并可原位恢复；专栏不提供任何不可恢复的删除。性能变化请用游戏自带基准或常用监控工具对照。
...
注销或关闭系统时会还原可持久化的改动（电源计划、注册表类开关）——这类改动重启后不会自动消失，所以必须在关机前处理。退出程序时按风险排序：先还原进程状态（优先级、亲和性、EcoQoS、核心分区、冻结），再还原耗时的服务与显示设置；退出预算被截断时，优先保住的是重启也无法自愈的那部分。
...
设置页原有的 Cross 永久删除入口已经移除。`Cross` 现在与其他附加层一样走可逆隔离，不再存在无法找回的删除路径。

可逆隔离处理 Cross、DiagnosticAssistant、FeedBack、NetworkAssist、TQM 和 TenioDL，将它们移入同盘隐藏仓库，先写清单再移动，不删除、不覆盖；英雄联盟或 WeGame 运行时拒绝操作，遇到目标冲突也不会覆盖已有文件。若客户端更新重新下载了某个组件导致恢复无法覆盖，可在确认每一项都已回到原位置后丢弃该批次记录并重新隔离。

运行时流程不注入进程、不修改内存、不改游戏核心文件，也不触碰 `League of Legends.exe`、Riot 核心、认证/补丁组件或 ACE。LCU 凭据只在内存中短暂使用，不写入日志、注册表、文件或子进程参数。文件层变化只来自用户单独确认的可逆隔离。专栏停用时不扫描磁盘、不枚举进程、不访问客户端接口；WeGame 以登录用户而非管理员身份启动，游戏与反作弊不会被连带提权。
...
构建脚本先生成图标，再编译带管理员权限清单、产品名称、公司名称和 `1.5.0.0` 文件版本的 `Aegis.exe`。
...
内置自测当前包含 `62` 项，覆盖 CPU 拓扑和高低核心数分级、CPU Sets 和硬亲和性恢复、PID 复用、真实子进程提优、分级压制、位号后缀兜底匹配的边界、跨焦点会话保护、前台和用户窗口保护、冻结看门狗、会话边界、英雄联盟凭据解析、精准净化边界、可逆隔离恢复与冲突保护、会话报告、发布元数据和界面离屏绘制。机器缺少对应能力时会记录 `SKIP`，不会算作 `PASS`。
