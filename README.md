...
**v2.2.1.3 · [更新说明](docs/releases/v2.2.1.3.md)**
...
游戏库选中游戏点独立配置进入。每个游戏可单独覆盖模式、后台压制、核心、内存电源、系统环境和显卡策略，当前策略目录共 33 项（含模式、家族压制和核心掩码）。没覆盖的项跟随全局，改动立即保存。另有逐 EXE 的全屏优化与 DPI 兼容设置，不计入这 33 项，也不随退局自动还原。
...
- **独立配置**：当前策略目录 33 项可逐游戏覆盖，没覆盖的跟随全局，条目支持重命名
...
- **候选线程提优**：默认开启，极限档强制开启；其它档位下帧率反而变低的游戏可以逐游戏关掉。掌机档和物理核不足 6 个的机器不提供——被提优的线程跑在优先级 15、游戏其余线程留在 13，可运行线程多于可用核心时它会抢占自己正在等的线程，帧率反而下降。按 CPU 耗时筛选候选，并不能证明它是帧关键线程；误选可能拖慢游戏。CPU 持续饱和时撤回候选提优，游戏进程回到普通优先级，不再因候选存在而跳过保护
...
- **手动选核**：逐核划分，预设有全核、关超线程、仅性能核、仅缓存核和反选，点应用才写入。对局中立即生效不必重启游戏，退场自动还原，不写注册表；少于两个逻辑核无法应用
