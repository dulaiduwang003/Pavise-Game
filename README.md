...
**v2.2.1.2 · [更新说明](docs/releases/v2.2.1.2.md)**
...
- **语音不压制**：正在使用麦克风的进程按音频采集会话识别，对局中一律不压制，不分档位，QQ 和微信通话时也靠这条放行，平时照常压制；KOOK、YY、TeamSpeak、Mumble、Oopz 和 DS4Windows、reWASD 等手柄映射器另按名字豁免。智能档的自适应升档改为独立开关，默认关闭

### 进程与核心

- **游戏进程提优**：渲染进程获得高优先级、更高的磁盘 IO、内存页和显卡调度优先级，退场还原
- **候选线程提优**：默认开启，极限档强制开启；其它档位下帧率反而变低的游戏可以逐游戏关掉。按 CPU 耗时筛选候选，并不能证明它是帧关键线程；误选可能拖慢游戏。CPU 持续饱和时撤回候选提优，游戏进程回到普通优先级，不再因候选存在而跳过保护
- **智能让位**：CPU 持续饱和十秒时，游戏本体暂回普通优先级并撤回候选线程提优；候选已提优也不能绕过保护。这是风险控制，不代表所有游戏必定提高帧率
...
- **游戏核心分区**：给游戏选择可运行的核心集合，适配混合架构、X3D 和多处理器组；不会因此把所有后台赶到剩余核心，也不保证游戏独占。后台绑核仅在单独启用的重压后台或反作弊策略符合条件时执行
...
国内对战平台单独列出：完美世界竞技平台、5E、B5。它们的反作弊内嵌在平台客户端里，Pavise 只保护不压制。网易 NEAC 补入 NeacClient 和 OWNeacClient，腾讯 TP 补入 TP3Helper；米哈游只有内核驱动，进识别表用于日志说明。

另有三组**仅保护**名单：PunkBuster、Nexon Game Security（BlackCipher 等）、Wellbia XIGNCODE3 / UNCHEATER。按进程名、名称前缀和专用目录豁免后台压制，覆盖 `.aes` 组件和目录内的辅助进程；这三组不提供压制开关。关闭反作弊专项总开关不会关闭后台豁免。[识别规则、来源与覆盖边界](docs/anti-cheat-coverage.md)。
...
- **对局切换厂商性能档**：通过 Lenovo Legion 和 ASUS ROG 的厂商接口把笔记本切到性能档，退局切回；默认关闭，按需开启，不受电竞档或极限档强制
...
只读检查本机能力，按硬件和采样结果生成结论，不承诺固定条数。每条标注依据等级：本机实测、台架实测、机制明确、未验证。检出异常配置后可手动修复时间片、网络限流、输入队列、有线路由跃点、FTH 容错堆及被显式关闭的显卡 MSI。显卡功耗与温度限制、中断 DPC/ISR 来源也在这里报告。

设置页提供清除全部配置，一次撤销 Pavise 做过的所有持久改动，包括已下架功能留下的历史改动。旁边的卸载 Pavise 在程序内一键完成：先停止运行并按恢复记录还原全部系统改动，再删除开机任务、托管电源方案、设置、数据目录与旧版本残留，执行完相当于从未安装过，最后只需删掉 Pavise.exe 本体。
...
下文记录旧实现的退出原因，不代表同名能力在当前版本仍被移除。当前入口以上面的功能列表为准；历史恢复代码也不代表功能仍可启用。恢复时机按各项实现区分：部分残留在启动时清退，其余由设置页“清除全部配置”按记录撤销。
...
旧版自动批量 MSI 调整、后台硬限帧和按位置屏蔽 CPU 0/1 等实现已移除。当前仍有针对显卡显式 `MSISupported=0` 的手动修复、驱动低延迟。缓存预热已完整退役，旧配置在加载时丢弃。后台限帧未恢复：驱动中的“后台”通常指失去焦点的应用，切出游戏后可能限制游戏本身。

独立核心解停泊覆盖已由托管方案中的停泊设置承接；当前显卡偏好分为待命预选和持久应用偏好，均不迁移运行中的进程。禁止 CPU 空闲为手动策略，不由极限预设自动开启。1.8.0.2 移除的是当时的极限模式和 LOL 专栏，当前已有重新实现的极限档和 LOL 扩展。IFEO 后备提优与逐游戏关闭 CFG 不再提供新写入，仅保留历史恢复。

RSS 收包移核、GPU 锁频、系统内存驻留、对局单屏、自动中断编排、进程冻结、自动刷新率守护、MPO 禁用和通知免打扰均不属于现行启用能力。系统内存驻留与仍保留的显存预留不是同一项。

`tools/` 下的核显协同、VRS、Thermal Exchange 与 Interrupt Fabric 是研发台架或探测器，没有接入任意游戏的通用优化路径；台架吞吐不能当作游戏 FPS 收益。
