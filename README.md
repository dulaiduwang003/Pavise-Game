...
- 内存清理只保留代价最小的一项，默认关闭，只建议内存紧张的机器开启，详见下一节
- 对局默认切到 Pavise 托管电源方案，首次对局自动创建，各项参数按本机处理器写入，名称带 PG 标签和本机签名；也可改选本机任意计划，Pavise 只负责切换不改其参数
...
只清系统判定最不可能再用到的那部分缓存页，耗时 13 毫秒，不动整个待机列表，因此不会把仍有价值的文件缓存一起丢掉。每局执行一次。内存紧张的机器加载会顺一点，内存充裕的基本无感知。
...
ISLC（Intelligent Standby List Cleaner）的等价实现，`待机列表 ≥ 1 GB 且可用内存 ≤ 1 GB` 的双条件比 ISLC 默认保守，但触发之后代价大于收益：1380 毫秒全系统阻塞加整个文件缓存丢弃，只换回 382 MB 可用内存。更关键的是没有冷却，5 秒一次检查——触发条件成立本就说明可用内存见底，清空后缓存被读盘重新填满，几秒内又会再次越线，低配机上会形成清空、读盘、再堆积的反复触发，比它想解决的周期性微卡更糟。
