...
| `GpuContentionBench.cs` | GPU 争抢台架：`victim` 测帧间隔、`burn` 满载 GPU、`setgpu <pid> <class> <out>` 用线上同款 `D3DKMTSetProcessSchedulingPriorityClass` 降级并回读。验证「后台 GPU 让位」的收益 |
