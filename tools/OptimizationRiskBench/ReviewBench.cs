// File purpose Review-only entry, scheduling counterexamples on self-owned processes plus read-only probes
// Bypasses Program.Main, no GameMode loop, Sweep or Boost, never touches real settings, power or driver writes
using System;
...
namespace PaviseApp
{
    internal static class OptimizationReviewBench
    {
        ...
        private static void Worker(string[] args)
        {
            ...
            using (var view = map.CreateViewAccessor())
            {
                ...
                try
                {
                    ...
                    // The production CPU core-domain gate, machine-wide spare CPU is not enough to decide on
                    ...
                    if (game)
                    {
                        ...
                        while (Stopwatch.GetTimestamp() < end)
                        {
                            ...
                            // Explicitly synthesized cross-process busy-wait dependency
                            // do not claim every game follows this pattern
        ...
        private static void Priority(string directory)
        {
            ...
            // Separate here means distinct logical CPUs, it says nothing about physical core mapping
        ...
        private static void PowerInputs()
        {
            ...
            // All samples are synthetic except the cold-start CpuSaturation.Sample
            // The laptop eligibility override bypasses the real runner and never touches EPP or telemetry hardware
        }
        ...
        private static void ReadCost()
        {
            ...
            {
                // The real production background query, including cold-start GPU inventory and PDH init
        ...
        {
            // Just one focused combined probe, not the live GPU/window/identity discovery loop
            // Calls the production path visibility gate first, then the production preference manager
            // The remaining external preconditions are explicitly hard-coded synthetic assumptions
