...
**v2.2.1.4 · [更新履歴（中国語）](docs/releases/v2.2.1.4.md)**
...
- **手動コア設定**：CCD ボタンや No HT を使ってゲームの論理 CPU を選択。必要に応じてコア隔離を有効にし、展開して物理コアを選びます。通常のバックグラウンドは隔離範囲を回避し、ゲーム提優中だけ適用して終了時に復元します。割り込み等は使用する場合があります。
- **CCD 選択**：CCD ボタンでゲームを実行するチップ領域を手動選択できます。
- **手動コア選択**：全選択・No HT・選択解除・反転・CCD ボタンに対応。論理 CPU を 2 つ以上選択してください。保存後、次のゲームから適用し、現在の対戦は維持します。終了時に元の親和性へ復元します。
...
**保護専用**は合計 6 グループです。上記の中国アリーナ 3 種に加え、PunkBuster、Nexon Game Security（BlackCipher など）、Wellbia XIGNCODE3 / UNCHEATER。プロセス名、名前の接頭辞、専用ディレクトリでバックグラウンド抑制から除外し、`.aes` コンポーネントや同ディレクトリ内の補助プロセスも対象とします。この 6 グループに抑制スイッチはなく、アンチチートのコア固定にも参加しません。アンチチートのマスタースイッチをオフにしても保護は継続します。[識別規則・出典・対応範囲（中国語）](docs/anti-cheat-coverage.md)。
...
- **アフィニティ設定を一括で戻す**：このページで行ったピン留めを領収書に従ってすべて取り消します。再起動後に反映されます
- 各デバイスには現在の状態を表示します。反映済み、再起動待ち、未収束、検証待ち、対戦提案、入力デバイス、およびシステム環境ページが管理しているかどうか
- 触る価値がない場合はページ側から指摘します。マルチメッセージの MSI-X デバイスはピン留めで並列度が落ちる可能性があり、StorPort の DPC は IO を発行した CPU に追随するためピン留めしても通常は効きません

### メモリと電源

- **キャッシュ予熱**：復活、既定はオフ、ゲーム別に設定可能。対戦が 90 秒間安定した後、低優先度でリソースを先読みします。1 対戦あたり最大 2 GiB、約 31 MiB/s。AC 電源・十分な空きメモリ・SSD が必要で、スタンバイキャッシュ消去が優先されます。対戦終了やメモリ不足で停止します。[詳細（中国語）](docs/cache-warm-up.md)
