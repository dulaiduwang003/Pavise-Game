...
極限には自動の巻き戻しがあります。解錠後にシステムが異常再起動を 2 回（Kernel-Power 41、BugCheck 1001、EventLog 6008）起こすと、次回の起動で解錠を自動的に解除し、台帳に基づいて環境項目を復元します。戻すのは Pavise が記帳した分だけで、利用者自身が有効にした設定には触れません。システムログを読めない場合は 0 回として扱い、ヒューズはあえて作動しません。個々の機能が自らヒューズを飛ばした場合は極限リストから外れるだけで、他の項目には影響しません。
...
- **候補スレッドの昇格**：携帯機モードと物理コア 6 未満の環境では提供しません。それ以外では既定オン、極限では強制オン。他のモードではゲーム別にオフにできます。CPU 時間による候補選択はフレームに重要なスレッドの証明ではなく、誤選択で遅くなる場合があります。以前の優先度規則に戻し、CPU 全体の飽和だけでは有効な候補昇格を撤回しません
