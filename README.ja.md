...
**v2.2.0.1 · [更新履歴（中国語）](docs/releases/v2.2.0.1.md)**
...
設定ページには全設定の消去があり、Pavise がこれまでに行った永続的な変更を、廃止済み機能が残したものも含めて一括で戻します。隣の Pavise をアンインストールは同梱の Pavise-Uninstall.cmd を起動し、アプリを終了して記録に従いシステム変更をすべて復元した上で、起動タスク、管理電源プラン、設定、データフォルダー、旧バージョンの残留物を削除します。実行後は一度もインストールしていない状態になります。アプリが開けないときはこのスクリプトを直接ダブルクリックしても構いません。
