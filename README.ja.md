...
**v2.1.3.3 · [更新履歴（中国語）](docs/releases/v2.1.3.3.md)**
...
抑制の強さと性能は比例しません。廃止した極限モードは高負荷シナリオで 71 にとどまり、競技モードの 93 を下回りました。これが削除の理由です。既定のプリセットは最も穏やかな設定です。

現在ある 4 段階ベンチの数値は v1.7.1 当時のものです。i7-9750H のノート PC で WeChat・Clash・QQ・音楽プレーヤーを常駐させ、クリーンな状態を 100 とした相対値で、3 シナリオ平均は Pavise なし 59、通常 60、競技 96 でした（通常と競技が現在のスマートと集中）。2.x のスケジューリングモデルは 1.7 と異なるため、この数値は桁の目安に留まり、2.x での再測定はまだ行っていません。
...
| モード | 抑制範囲 |
|---|---|
| スマート | 対戦開始と同時に、保護境界を通過したバックグラウンドを一度にすべて隔離します。負荷は見ませんし、段階的に引き上げることもしません。使用中のプログラムとその一族は抑制の対象外です。CPU 飽和が続く場合（90% 超が 10 秒以上）は一時的に集中モードの基準へ引き上げ、負荷が回復して 2 分間安定すれば自動で戻します。1 対戦につき最大 3 回 |
| 集中 | 隔離範囲をゲーム以外の全体に広げます。ウィンドウを持つものも例外ではなく、ゲームから切り替えて使うプログラムも降格します。例外はホワイトリストのみ |
| 軽負荷 | バックグラウンド抑制は集中と同一で、電力側はベンダーツールに任せます。電源スライダーを操作せず、AC 接続時も純粋な省電力項目を有効のままにします。バッテリーが必要で、携帯機や薄型ノート向け |
...
ライブラリでゲームを選び、個別設定を開きます。ゲームごとにモード・バックグラウンド抑制・コア・メモリと電源・システム環境・グラフィックスの計 33 項目を上書きできます。上書きしていない項目は全体設定に従い、変更は即座に保存されます。
...
- **個別設定**：33 項目をゲームごとに上書きでき、上書きしていない項目は全体設定に従います。項目名の変更も可能です
...
- **抑制範囲と強度の引き上げ**：ゲーム以外のバックグラウンドを最大限に抑制し、切り替え後も解放しません。集中と軽負荷では固定でオン。IME や周辺機器ツールはホワイトリストへ
...
- **NVIDIA**：電源管理を最大パフォーマンス、低遅延（オンまたはウルトラ）、Smooth Motion フレーム生成、シェーダーキャッシュの上限解放、DLSS 上書き（最新または J・K 世代の指定）、ゲームごとの ReBAR
- **AMD**：Anti-Lag、AFMF フレーム生成、RSR ドライバーレベルアップスケーリング
- **Intel**：グローバル低遅延。ドライバーがリアルタイム切替に対応すると報告する DX9/DX11 パスでのみ有効化し、Boost と XeSS には触れません
...
- **CPU アイドルの無効化**：ゲーム中・AC 電源が確認できる・管理プランを使用している場合にのみ動作し、変更するのは AC 値のみです
- **スタンバイメモリ整理**：リストサイズと実際の空きメモリの両方がしきい値を超えた場合にのみスタンバイリスト全体を消去します。技術的な詳細は後述します
- **MMCSS マルチメディアスケジューリング**：非マルチメディア処理の予約枠を 20% から 10% に変更し、Games タスクのスケジューリングカテゴリーとファイル IO を引き上げます
...
- **ゲームモード保護**、**ウィンドウ表示ゲームの最適化**
- **タイマーの恒常ティック**、**グローバルタイマー精度**
- **NIC の省電力切断の禁止**、**NIC 割り込みモデレーションの無効化**（物理有線アダプターの標準化キーのみ）
- **アクセシビリティキーの遮断**、**キーボード・マウスの選択的サスペンドの無効化**

### 実験機能

8 つあり、すべて既定でオフ、有効化前に確認があり、ゲームごとに上書きできます。書き込みは毎回再読込で確認し、検証に失敗した場合は取り消して以後の試行を停止し、対戦終了時または無効化時に復元します。

- **ビデオメモリ常駐**：VRAM が逼迫した際に最小限の予約を宣言し、テクスチャが追い出されて再読み込みされることで生じる長いフレームを減らします。VRAM を増やすことも固定することもありません
- **メモリ常駐**：物理メモリの逼迫が続く場合にワーキングセットの下限を固定し、メモリ整理でゲームのページが回収されないようにします。メモリは割り当てず、16 GB 未満の機体は対象外です
- **キャッシュ予熱**：対局が安定した後、最低ディスク優先度でゲームの資産をスタンバイキャッシュへ先読みします。読み取りのみで、AC 電源・十分な空きメモリ・SSD のゲームドライブが条件です
- **対局シングルディスプレイ**：メイン画面のみに切り替え、誤クリックによるフォーカス喪失を防ぎ、サブ画面の合成も節約します。終了時に戻し、切替の瞬間は一時的に暗転します
- **電力譲渡**：ノート PC で GPU が上限に張り付き CPU に余裕があるとき、共有電力予算を GPU へ回します。検証方法は後述します
- **自動割り込みオーケストレーション**：複数対戦の実測で競合が確認されたデバイスの割り込みをゲームコアの外へ固定します。領収書を残し再起動後に有効となり、さらに 3 対戦で検収して無効ならロールバックします
- **バックグラウンドアプリの自動省電力 GPU**：ゲームの描画 GPU 上でまだ 3D を実行しているバックグラウンドアプリを、次回起動時に省電力 GPU を使うよう登録します。1 対戦につき最大 2 件
- **Intel グローバル低遅延**：上記グラフィックスの項を参照
